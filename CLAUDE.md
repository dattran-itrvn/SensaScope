...
(Append findings here. Format: ` - YYYY-MM-DD: short fact + one line of context.`)
 ...
 - 2026-05-14: **Apple Core Bluetooth refuses DLE (Data Length Extension) by policy** — confirmed via HCI DBG log on Zephyr peripheral. Peripheral sends `HCI_LE_Set_Data_Length` (opcode `0x2022`) with 251/2120us → over-air `LL_LENGTH_REQ` → Apple replies `LL_LENGTH_RSP` with 27/2120us → Zephyr fires `le_data_len_change` event with `max.tx: 27 (2120us), max.rx: 27 (2120us)`. Both macOS (Bleak/PyObjC) and iOS (nRF Connect, iOS 17+) behave identically. Math: LL=27 × ~8-10 packets/event × 67-90 events/sec ≈ 14-17 KB/s ceiling regardless of GATT vs L2CAP CoC vs PHY 2M. **No firmware-side workaround** — Apple's stack drops `LL_LENGTH_REQ` upgrade for power/policy reasons. To break the ceiling: change host (Linux/Android, DLE works → ~80 KB/s), reduce data volume (mono / lower sample rate), compress on FW (ADPCM 4:1), or change transport (USB / WiFi). L2CAP CoC firmware code is preserved on main (PSM `0x0080`) as a +14% improvement and the right architecture if a non-Apple host shows up.

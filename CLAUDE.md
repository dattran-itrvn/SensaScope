...
Hard rules:
- Device states: IDLE / RECORDING / SYNC / LOW_BATT_HOLDOFF / ERROR.
- **BLE advertising runs in ALL states** (v1.1.2 redesign — see Discovered 2026-05-22). The advertiser is started once in `start_ble()` and restarted in `on_disconnected()` regardless of FSM state.
- **Persistent BLE connection ONLY in SYNC** (i.e., the device was IDLE/LOW_BATT/ERROR before connect). During RECORDING the link is allowed only as a brief opcode exchange: PC connects → sends one opcode → device replies → device self-disconnects in ~150 ms via `schedule_self_disconnect()` in `ble_sync.c`. Never hold a link across a recording session — bench confirmed this causes BT/PDM/SD resource contention that crashes the writer (#36).
- **Recording ownership** (v1.1.2): `enum rec_owner { NONE, TAP, BLE }` enforces start-method=stop-method symmetry. Tap-owned record stops only by tap; BLE-owned stops only by `OP_STOP_RECORD`. BLE disconnect mid-record does NOT auto-stop — recording continues until explicit STOP, batt low, SW1 off, or SD full.
...
(Append findings here. Format: ` - YYYY-MM-DD: short fact + one line of context.`)
 ...
 - 2026-05-14: **Apple Core Bluetooth refuses DLE (Data Length Extension)** — *internal bench finding, NOT cited from Apple-published documentation.* Confirmed via HCI DBG log on Zephyr peripheral: peripheral sends `HCI_LE_Set_Data_Length` (opcode `0x2022`) with 251/2120us → over-air `LL_LENGTH_REQ` → Apple replies `LL_LENGTH_RSP` with 27/2120us → Zephyr fires `le_data_len_change` event with `max.tx: 27 (2120us), max.rx: 27 (2120us)`. Both macOS (Bleak/PyObjC) and iOS (nRF Connect, iOS 17+) behave identically. The throughput ceiling 14-17 KB/s comes from our math (LL=27 × ~8-10 packets/event × 67-90 events/sec), NOT from any Apple statement. The phrase "for power/policy reasons" in earlier drafts of this note was speculation — Apple has not publicly documented why its stack refuses DLE upgrade, and the "Bluetooth Accessory Design Guidelines for Apple Products" (Apple's public BLE accessory doc) does not specify DLE behaviour or throughput limits. To break the ceiling without an Apple statement: change host (Linux/Android, DLE works → ~80 KB/s bench), reduce data volume (mono / lower sample rate), compress on FW (ADPCM 4:1), or change transport (USB / WiFi). L2CAP CoC firmware code is preserved on main (PSM `0x0080`) as a +14% bench improvement and the right architecture if a non-Apple host shows up.
 - 2026-05-22: **Confirmed sustained BLE GATT throughput = 15.5 KB/s** via on-disk file-size sampling during sync of SESSION_00008 (1.78 MB audio.wav). Driver `/tmp/sync_throughput_probe.py` polls `<tmp>/audio.wav` size every 5s while `tools/sync.py` runs as subprocess. 25 samples between t=20s and t=125s show **77,520 B per 5s window** — that's 319 chunks × 243 B/chunk @ ~64 chunks/sec, matching Apple's coerced ~30 ms connection interval × multi-credit notify. Variation: occasional 81,600 B (1 extra chunk landing in window). The "+40 KB/s burst" mentioned in an earlier sync9 interim observation was a **measurement artefact** — comparing on-disk size at +99s wall-clock vs sync-start log timestamp double-counted setup overhead (scan + connect + LIST takes ~10s before first chunk lands). Net: there is no burst-then-throttle behaviour — Apple link is rock-steady at 15.5 KB/s from first audio byte. This matches the 2026-05-14 derived ceiling 14-17 KB/s within rounding.
 - 2026-05-22: **v1.1.1 BLE-during-RECORDING crashed the writer at ~64 s** — bench repro: PC ble-start, link held alive by sync.py `--hold`, watchdog tripped FSM ERROR (SOS) ~64s in. Root cause: v1.1.1 deliberately kept the BLE link active during BLE-driven RECORDING (so PC could send OP_STOP_RECORD over the same connection). This broke the CLAUDE.md hard rule "RECORDING turns BLE controller off" — Apple's ~30 ms connection interval drives ~33 events/sec of LL traffic + GATT keep-alive in parallel with PDM IRQs at 16 kHz and the SD writer's SPIM3 DMA. The contention killed `sd_writer` after ~3-4 MB written (matches the 3.98 MB stub on SESSION_00002). **Fix v1.1.2** (2026-05-22): redesigned the BLE/FSM coupling — BLE advertising runs in all states, persistent link only in SYNC; during RECORDING the PC connects briefly for opcode exchange and the device self-disconnects ~150 ms after each reply (`schedule_self_disconnect()` in `ble_sync.c`). On-disconnect auto-stop removed — recording survives PC vanish. `enum rec_owner` replaces the bool `recording_via_ble` and is cleared at every FSM exit from RECORDING (watchdog abort, batt-low auto-stop, tap stop, BLE stop). OP_RESET_CTL whitelisted alongside OP_STOP_RECORD during RECORDING (it's a local-state-only op; `tools/sync.py` setup() issues it routinely so blocking it broke STOP). Bench verified: 100 s sustained BLE-driven recording with no link held, 0 SOS, 0 watchdog trips, session 9 = 7.1 MB clean. Also: tap threshold lowered 0x14 → 0x10 (~1.25 g → ~1.0 g) per #22's documented fallback — safe to lower now because sleep-wear sessions are BLE-owned so accidental motion taps are ignored.
 - 2026-06-08: **PDM L/R channels were SWAPPED — body landed on ch1, ambient on ch0** (the entire project history shipped with this). Found by bench tap-test: tapping the body mic clipped WAV **ch1**, tapping the ambient mic clipped **ch0** (SESSION_00002). Root cause: `app/src/audio.c` used the driver's `def_map` (ch0=LEFT, ch1=RIGHT) → `NRF_PDM_EDGE_LEFTFALLING`, under which the ST mics' L/R-via-clock-edge convention comes out inverted. Fix: `alt_map` (ch0=RIGHT, ch1=LEFT) → `NRF_PDM_EDGE_LEFTRISING` (the driver's documented L/R swap, `dmic_nrfx_pdm.c:399`). Verified SESSION_00003: body taps now clip ch0. **Consequence:** every v2 notebook + all recordings before this fix have swapped channels — they analysed the AMBIENT mic as "body" and used the BODY mic as the noise reference, which is why every NLMS/ablation result was ambiguous/contradictory. Offline fix is centralised in `tools/load_session.py` (`legacy_channels=True` default un-swaps) + `nlms_denoise --fixed-channels`. meta.json hash can NOT distinguish eras (fix shipped uncommitted, same hash 3fb40d2). **Always re-verify channel mapping with a tap-test after any PDM/pinctrl change.** Commits `823706d` (FW) + `c1e5471` (tools) on branch `fix/pdm-channel-swap`.
 - 2026-06-08: **Body mic (bare MEMS on skin) couples heart/lung POORLY** — separate from the swap. Even on the correctly-identified body channel, the cardiac signal is low-amplitude (lung-band rms ~4 vs ambient ~32) and **inaudible by ear at max headphone volume**, though a clean 72 bpm periodicity is detectable. Body-mic energy is dominated by 20-200 Hz rumble (contact/motion, around the 130 Hz HPF) that headphones barely reproduce. Also: the two mic channels are strongly **anti-phase coherent** in normal recording (corr ≈ −0.93 @ 100-1000 Hz, ~0 µs lag) — structure-borne body sound reaching both mics back-to-back on the rigid PCB, NOT digital crosstalk (broadband coherence tracks real acoustic content, would be flat if electrical). Implication for PCB v2 §13: body mic needs a mechanical acoustic coupler (bell/diaphragm chamber) + the ambient mic needs mechanical isolation from the PCB before dual-mic ANC can work — the current layout shares the signal into both mics so any ambient-reference canceller removes signal too.
 - 2026-06-08: **SD `io_err` on START after a card hot-swap — firmware does NOT remount on card insert**, even though CARD_DETECT (P0.26) exists. Repro: pull card → erase/reinsert → BLE START_RECORD returns `status=io_err`; LIST shows stale free space. Fix = reboot (SW1 OFF/ON or reflash) so `sdlog_init` re-runs `disk_access_init`. Bench J-Link could not `--reset`/attach while running on battery (cable disturbed by handling) — so SW1 power-cycle is the reliable field reboot. Same session also hit `io_err` mid **BLE READ** at ~5.4 MB of a 15.3 MB sync (data was fully intact + readable on the Mac afterward → FATFS-read-under-BLE-load issue, not a dead card). Track as a follow-up: device-side remount-on-CARD_DETECT + SD-read robustness under BLE. Apple BLE throughput this session measured **~8.7 KB/s** — about HALF the 2026-05-22 figure of 15.5 KB/s; host negotiated a slower (~60 ms) connection interval, confirming the rate is host/negotiation-dependent and non-deterministic, not a fixed ceiling.
